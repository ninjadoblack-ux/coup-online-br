        ...
        if (playersRes.data) {
          ...
          if (currentMe) {
            ...
            const [cardsRes, logsRes, actionsRes] = await Promise.all([
              ...
              supabase.from('game_actions').select('*').eq('room_id', roomId).in('status', ['pending', 'challenged', 'blocked', 'blocking', 'block_challenged', 'awaiting_reveal', 'exchanging', 'executing_final'])
            ]);
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'players', filter: `room_id=eq.${roomId}` }, 
        payload => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            if (updatedPlayer.user_id && myPlayer && updatedPlayer.id === myPlayer.id) {
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'game_actions', filter: `room_id=eq.${roomId}` }, 
        payload => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            const activeStatuses = ['pending', 'challenged', 'blocked', 'blocking', 'block_challenged', 'awaiting_reveal', 'exchanging', 'executing_final'];
