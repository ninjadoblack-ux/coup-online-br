        ...
        if (playersRes.data) {
          ...
          if (currentMe) {
            ...
            const [cardsRes, logsRes, actionsRes] = await Promise.all([
              ...
              supabase.from('game_actions').select('*').eq('room_id', roomId).in('status', ['pending', 'challenged', 'blocked', 'blocking', 'block_challenged'])
            ]);
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'game_actions', filter: `room_id=eq.${roomId}` }, 
        payload => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            const activeStatuses = ['pending', 'challenged', 'blocked', 'blocking', 'block_challenged'];
